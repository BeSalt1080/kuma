...
const wishlist = async (cart) => {
    if (cart.wishlist) {
        ...
        await authService
            .post('/wishlist', { products_id: cart.id }, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                }
            })
            ...
            <div class="px-10">
                <div class="flex justify-between">
                    <div class="w-1/2">
                        ...
                        <div class="flex w-full border-b my-2" v-for="(cart) in carts" :key="cart.id">
                            ...
                            <div class="w-full">
                                ...
                                <div>
                                    Size: {{ cart.size }}
                                    Quantity: <button>-</button> <input class="w-10" type="text" :value="cart.quantity">
                                    <button>+</button>
                                </div>
                                <div class="flex gap-5">
                                    <button class="w-10 h-10" @click="wishlist(cart)">
                                        <i class="fa-regular fa-heart text-2xl mt-1"
                                            :class="{ 'text-red-600 fa-solid': cart.wishlist }"></i>
                                    </button>
                    ...
                    <div class="w-1/3">
                        ...
                        <hr>
                        <router-link :to="{name:'order.checkout'}" class="block text-center mt-5 p-4 bg-green-400 hover:bg-green-300 w-5/6 mx-10">Checkout</router-link>
                    </div>
                </div>
